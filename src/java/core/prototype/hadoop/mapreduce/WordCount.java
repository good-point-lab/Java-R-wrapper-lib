...
public class WordCount implements MapreduceDelegateApi {

    private JobConf jconf;

    public WordCount(JobConf jconf) {
        this.jconf = jconf;
    }
    ...
    public void runJob(String inputPath, String outputPath) {
        try {
            ...
            jconf.setMapperClass(WordCount.Map.class);
            ...
            FileInputFormat.setInputPaths(this.jconf, new Path(inputPath));
            FileOutputFormat.setOutputPath(this.jconf, new Path(outputPath));
            JobClient.runJob(this.jconf);
        } catch (Exception ex) {
